/* InvoKit Pro v2.0 — Main Application Logic */
    ...
    d.innerHTML = `
      <div style="position:relative">
        <input type="text" placeholder="Item description" value="${eh(row.desc)}"
          maxlength="150"
          oninput="eState.rows[${i}].desc=this.value;rd();showIAC(${i},this.value)"
...
function addRow()  { eState.rows.push({desc:'',qty:1,price:0,tax:0}); renderRows(); rd(); }
function delRow(i) { eState.rows.splice(i,1); renderRows(); rd(); }
...
async function saveInvoice() {
  ...
  if (!num) { toast('Enter an invoice number','er'); return; }
  const cn  = gv('e-cname'); if (!cn) { toast('Client name is required','er'); return; }
  const hasRow = eState.rows.some(r => (r.desc||'').trim() !== '');
  if (!hasRow) { toast('Add at least one line item','er'); return; }
...
async function exportPDF() {
  ...
  host.style.cssText = 'position:fixed;left:-99999px;top:0;width:794px;height:auto;background:#fff;z-index:-1;overflow:visible;';
  ...
  try {
    ...
    const opt = {
      ...
      html2canvas:{ scale:2, useCORS:true, logging:false, backgroundColor:'#ffffff', scrollX:0, scrollY:0, windowWidth:794 },
...
#doc{
  ...
  display:flex;flex-direction:column;font-family:'Segoe UI', system-ui, -apple-system,sans-serif;
  flex-shrink:0;
}
...
.lpill{display:flex;flex-direction:column;align-items:center;gap:5px;padding:8px 3px 7px;border:1.5px solid var(--bd2);border-radius:8px;background:var(--sur2);cursor:pointer;transition:.14s;color:var(--muted);font-family:'Segoe UI',system-ui,sans-serif;width:100%;}
...
async function renderInvoices(search) {
  if (search === undefined) search = gv('inv-search');
  const sf = document.getElementById('inv-status-f') ? document.getElementById('inv-status-f').value : '';
  let all = await dbAll('invoices');
  if (search) all = all.filter(i => i.num?.toLowerCase().includes(search.toLowerCase()) || i.clientName?.toLowerCase().includes(search.toLowerCase()));
  if (sf) all = all.filter(i => i.status===sf);
  all.sort((a,b) => b.savedAt-a.savedAt);
  const tb = $('inv-tbody');
  if (!all.length) { tb.innerHTML=`<tr><td colspan="7" style="text-align:center;padding:22px;color:var(--muted);font-size:.8rem;">No invoices found</td></tr>`; return; }
  tb.innerHTML = all.map(inv => {
    const c = CURS[inv.currency||'GBP']||CURS.NGN;
    return `<tr>
      <td class="mono">${eh(inv.num)}</td><td>${eh(inv.clientName)}</td>
      <td class="mono">${eh(inv.issueDate||'—')}</td><td class="mono">${eh(inv.dueDate||'—')}</td>
      <td class="r mono">${c.s}${(inv.total||0).toLocaleString(c.l,{minimumFractionDigits:2,maximumFractionDigits:2})}</td>
      <td><span class="badge b-${inv.status||'draft'}">${inv.status||'draft'}</span></td>
      <td><div style="display:flex;gap:4px;">
        ${inv.status !== 'paid' ? `<button class="btn btn-ok btn-xs" onclick="markAsPaid(${JSON.stringify(inv.id)})" title="Mark as Paid" style="background:#059669;color:#fff;border-color:#059669;">✓ Paid</button>` : ''}
        <button class="btn btn-o btn-xs" onclick="loadInvoice(${JSON.stringify(inv.id)})">Edit</button>
        <button class="btn btn-o btn-xs" onclick="printInvoiceById(${JSON.stringify(inv.id)})">Print</button>
        <button class="btn btn-a btn-xs" onclick="exportPDFById(${JSON.stringify(inv.id)})">PDF</button>
        <button class="btn btn-g btn-xs" onclick="openReceiptFromInvoice(${JSON.stringify(inv.id)})" title="Record payment for this invoice">💳 Paid</button>
        <button class="btn btn-d btn-xs" onclick="deleteInvoice(${JSON.stringify(inv.id)})">Del</button>
      </div></td>
    </tr>`;
  }).join('');
}
...
async function saveClient() {
  ...
  try {
    ...
    await dbPut('clients', obj); await updateCounts(); renderClients();
    closeM('m-client'); toast('Client saved ✓','ok');
  } catch(e) { toast('Save failed — please try again','er'); }
  finally { if (btn) { btn.disabled = false; btn.textContent = orig; } }
}
async function deleteClient(id) {
  confirm2('Delete Client','Remove this client?', async () => { await dbDel('clients',id); await updateCounts(); renderClients(); toast('Deleted','er'); });
}
...
async function renderItems(search='') {
  ...
  const tb = $('itm-tbody');
  if (!all.length) { tb.innerHTML=`<tr><td colspan="5" style="text-align:center;padding:22px;color:var(--muted);">No items yet.</td></tr>`; return; }
...
async function deleteItem(id) {
  confirm2('Delete Item','Remove this item?', async () => { await dbDel('items',id); renderItems(); toast('Deleted','er'); });
}
...
document.querySelectorAll('.overlay').forEach(o => o.addEventListener('click', e => { if(e.target===o && o.id!=='m-ob') o.classList.remove('on'); }));
...
async function bkpBoot() {
  try {
    ...
    // 2. Previously activated in this browser — key stored in localStorage
    const storedKey = bkpLoadStoredKey();
    if (storedKey) {
      const payload = await bkpVerify(storedKey);
      if (payload) { bkpApplyLicence(payload); return true; }
      localStorage.removeItem(BKP_STORE_K); // corrupt/tampered — clear it
    }
...
function _bkpShowLock() {
  ...
  if (lock) {
    ...
    // Show sales page first, activation panel hidden
    document.getElementById('bkp-sales').style.display = 'flex';
    document.getElementById('bkp-activate-panel').style.display = 'none';
  }
}
function bkpShowActivate() {
  document.getElementById('bkp-sales').style.display = 'none';
  const panel = document.getElementById('bkp-activate-panel');
  panel.style.display = 'block';
  panel.style.animation = 'sfade .2s ease';
  setTimeout(() => document.getElementById('bkp-key-inp')?.focus(), 120);
}
function bkpShowSales() {
  document.getElementById('bkp-activate-panel').style.display = 'none';
  document.getElementById('bkp-sales').style.display = 'flex';
}
...
function bkpSaveLicence(key) {
  localStorage.setItem(BKP_STORE_K, key);
}
function bkpLoadStoredKey() {
  return localStorage.getItem(BKP_STORE_K) || null;
}
...
function bkpApplyLicence(payload) {
  ...
  // Lock business name to licence org if present
  if (payload.o) {
...
// write a  tag with id into the page and point the SW at that script's src,
...
async function renderReceipts() {
  ...
  if (!tbody) return;
  if (!receipts.length) {
    tbody.innerHTML = '<tr><td colspan="5" style="text-align:center;padding:30px;color:var(--muted);">No receipts yet. Record your first payment!</td></tr>';
    return;
  }
